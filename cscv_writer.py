...
failure_flag = 0
trial_index = 0
features_filepath = r'C:\Users\kpalac\OneDrive - Brown University\Documents\sims_take_3.csv'
labels_filepath = r'C:\Users\kpalac\OneDrive - Brown University\Documents\sims_take_4.csv'
...
failed_earlystop = []
num_sims = 2
with open(features_filepath, 'w', newline='') as features_file, \
     open(labels_filepath, 'w', newline='') as labels_file:
    
    features_writer = csv.writer(features_file)
    labels_writer = csv.writer(labels_file)

    # Write headers for the features CSV
    features_writer.writerow([f'Simulation_{i + 1}' for i in range(num_sims)])
##### Breast Parameters (FIXED) #####
    ...
    while current_index <= num_sims:
        ...
        try:
            ...
            # U2_rows = [[value] for value in U2_ave] 
            # features_writer.writerows(U2_rows) # Write averaged U2 data to CSV
            for value in U2_ave:
                features_writer.writerow([value])

            # Write labels
            labels_writer.writerow([f'Simulation {current_index}'])
            labels_writer.writerow([E_tumor])
            labels_writer.writerow([tumorSize])
            labels_writer.writerow([p_tumor])
            labels_writer.writerow([v_tumor])
    ...
    # features_transposed = list(map(list, zip(*all_features)))
    # features_writer.writerows(features_transposed)
